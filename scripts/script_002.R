...
suffled_rows=sample(nrow(train))
train=train[suffled_rows,]
train$ca[is.na(train$ca)]=0
train$trestbps[is.na(train$trestbps)]=mean(train$trestbps)
train$thalach[is.na(train$thalach)]=sample(train$thalach,1)
train$thal[is.na(train$thal)]=sample(train$thal,1)

csv1=train[1:81,]
csv2=train[(81+1):(81*2),]
csv3=train[((81*2)+1):(81*3-1),]


####

tran_cv1=rbind(csv1,csv2)
val_cv1=csv3
tran_cv2=rbind(csv1,csv3)
val_cv2=csv2
tran_cv3=rbind(csv2,csv3)
val_cv3=csv1
...
for (col in n){
  ...
  glm=glm(formula1,tran_cv1,family=(binomial(link="logit")))
...
formula=as.formula("target~sex+cp+trestbps+chol+thalach+exang+ca")
glm_stepwise=glm(formula,tran_cv1,family=(binomial(link="logit")))
prediccion_cv1=predict(glm_stepwise,val_cv1,type="response")


prediccion_cv1=as.vector(prediccion_cv1)
test=cbind(val_cv1,prediccion_cv1)
...
estim2=vector()
pval2=vector()
n2=colnames(data)[colnames(data)!="target"]

for (col in n2){
  formula1=as.formula(paste0("target~",col))
  glm=glm(formula1,tran_cv2,family=(binomial(link="logit")))
  estim2=append(estim2,as.vector(glm$coefficients[2]))
  pval2=append(pval2,summary(glm)$coefficients[2,4])
  print(summary(glm)$coefficients[2,4])
}

datos_glm2=rbind(estim2,pval2)
colnames(datos_glm2)=n2


glm_stepwise2=glm(formula,tran_cv2,family=(binomial(link="logit")))
prediccion_cv2=predict(glm_stepwise,val_cv2,type="response")
...
estim3=vector()
pval3=vector()
n3=colnames(data)[colnames(data)!="target"]

for (col in n3){
  formula1=as.formula(paste0("target~",col))
  glm=glm(formula1,tran_cv3,family=(binomial(link="logit")))
  estim3=append(estim3,as.vector(glm$coefficients[2]))
  pval3=append(pval3,summary(glm)$coefficients[2,4])
  print(summary(glm)$coefficients[2,4])
}

datos_glm3=rbind(estim3,pval3)
colnames(datos_glm)=n3

formula=as.formula("target~sex+cp+trestbps+chol+thalach+exang+ca")
glm_stepwise3=glm(formula,tran_cv3,family=(binomial(link="logit")))
prediccion_cv3=predict(glm_stepwise,val_cv3,type="response")
